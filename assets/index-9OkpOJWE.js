(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))s(n);new MutationObserver(n=>{for(const o of n)if(o.type==="childList")for(const d of o.addedNodes)d.tagName==="LINK"&&d.rel==="modulepreload"&&s(d)}).observe(document,{childList:!0,subtree:!0});function r(n){const o={};return n.integrity&&(o.integrity=n.integrity),n.referrerPolicy&&(o.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?o.credentials="include":n.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function s(n){if(n.ep)return;n.ep=!0;const o=r(n);fetch(n.href,o)}})();const L="https://image.tmdb.org/t/p/w1920_and_h800_multi_faces/",E="https://media.themoviedb.org/t/p/w200",C="",x="https://api.themoviedb.org/3",M="movie/popular",U="search/movie",y="data:image/png;base64,";class Y{movie;constructor(t){this.movie=t}render(){return`<div class="item">
      <img
         class="thumbnail"
         src="${E}${this.movie.poster_path}"
         alt="${this.movie.title}"
        />
        <div class="item-desc">
          <p class="rate">
            <img src="${y}" class="star" />
            <span>${this.movie.vote_average.toFixed(1)}</span>
          </p>
          <strong>${this.movie.title}</strong>
        </div>
      </div>
    `}}class b{render(){return`<div class="movie-skeleton">
      <div class="movie-skeleton__poster"></div>
      <div class="movie-skeleton__details">
       <div class="movie-skeleton__title"></div>
       <div class="movie-skeleton__info"></div>
      </div>
    </div>
    `}}const R="/javascript-movie-review/assets/Nosearch-9C2OvmM-.png";class h{movieList;movieContainer;constructor(){this.movieList=document.querySelector(".thumbnail-list"),this.movieContainer=document.querySelector(".main-result")}showEmpty(){this.movieContainer.innerHTML=`
      <div class="result-none">
        <img src="${R}" alt="검색 결과 없음" class="result-none-image" />
        <p class="result-none-text">검색 결과가 없습니다.</p>
      </div>
    `}clearList(){this.movieList.innerHTML="",this.movieContainer.innerHTML=""}renderSkeleton(){this.clearList();for(let t=0;t<20;t++){const r=document.createElement("li"),s=new b;r.innerHTML=s.render(),this.movieList?.append(r)}}renderMovieList(t){t.results.forEach(r=>{const s=document.createElement("li"),n=new Y(r);s.innerHTML=n.render(),this.movieList?.append(s)})}}const V="data:image/png;base64,",O="data:image/png;base64,",A={clearHeader(){const e=document.querySelector(".background-container");e.innerHTML="",e.style.background="none",e.style.height="100px"},render(e){const t=document.querySelector(".background-container");t.innerHTML=`
            <div class="top-rated-movie">
                <div class="rate">
                  <img src="${y}" class="star" />
                  <span class="rate-value">${e.vote_average.toFixed(1)}</span>
                </div>
                <div class="title">${e.title}</div>
                <button class="primary detail">자세히 보기</button>
                </div>
            </div>
            ${this.renderImage()}
      `,t.style.background=`url(${L}${e.backdrop_path}) no-repeat center center / cover`,t.style.removeProperty("height")},renderSearch(){const e=document.querySelector(".background-container");e.innerHTML=`${this.renderImage()}`},renderImage(){return`<div class="overlay" aria-hidden="true">
                <div class="search-container">
                <h1 class="logo">
                    <img src="${V}" alt="MovieList" />
                </h1>
                <form class="search-form">
                    <input
                    type="search"
                    class="search-input"
                    placeholder="검색어를 입력하세요"
                    />
                    <button type="submit" class="btn-submit">
                        <img
                            src="${O}"
                            alt="search"
                            class="img-search"
                        />
                     </button>
                 </form>
                </div>
            </div>`}},p=new h;function j(){A.clearHeader(),A.renderSearch()}function I(e){A.clearHeader(),A.render(e)}function f(e){p.clearList(),p.renderMovieList(e)}async function g(e,t,r=""){const s=r===""?"":`&query=${encodeURIComponent(r)}`,n=await fetch(`${x}/${e}?api_key=${C}${s}&language=ko-KR&page=${t}`);if(!n.ok)throw new Error(`API 요청 실패: ${n.status}`);return await n.json()}const S=new h;async function v(e,t){return t?await g(U,e,t):await g(M,e,t)}async function N(e,t){try{return await v(e,t)}catch{throw new Error("영화 데이터를 불러오는 중 오류가 발생했습니다.")}}async function Q(e){try{return S.renderSkeleton(),await v(e)}catch{throw new Error("영화 데이터를 불러오는 중 오류가 발생했습니다.")}}async function B(e,t){try{return S.renderSkeleton(),await v(e,t)}catch{throw new Error("영화 데이터를 불러오는 중 오류가 발생했습니다.")}}const w=new h;async function u(e,t,r){const s=document.querySelector(".main-title");s.textContent="지금 인기 있는 영화";const n=await Q(e);I(n.results[0]),f(n),r(t,n)}async function k(e,t,r,s){const n=document.querySelector(".main-title");n.textContent=`"${t}" 검색 결과`;const o=await B(e,t);j(),o.results.length===0?w.showEmpty():f(o),s(r,o)}async function G(e,t,r,s){const n=await N(e,t);w.renderMovieList(n),s(r,n)}let i=1,c="";const a=document.querySelector(".btn-more");addEventListener("load",async()=>{await u(i,a,l),document.querySelector(".background-container").addEventListener("submit",async t=>{if(t.preventDefault(),i=1,c=document.querySelector(".search-input").value.trim(),c===""){await u(i,a,l);return}await k(i,c,a,l),m()}),m()});function m(){document.querySelector(".logo").addEventListener("click",async()=>{i=1,c="",await u(i,a,l),m()})}a.addEventListener("click",async()=>{i+=1,await G(i,c,a,l)});function l(e,t){t.total_pages===i?e.style.display="none":e.style.display="block"}
